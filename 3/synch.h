...
class ConditionPriority {//改条件变量，wait时入队时带优先级，给AlarmClock使用
    public:
        ConditionPriority(char * debugName);
    ~ConditionPriority();			// deallocate the condition
    char* getName() { return (name); }
    
    void Wait(Lock *conditionPriorityLock, int p); 	// these are the 3 operations on 
					// condition variables; releasing the 
					// lock and going to sleep are 
					// *atomic* in Wait()
    void Signal(Lock *conditionPriorityLock);   // conditionLock must be held by
    void Broadcast(Lock *conditionPriorityLock);// the currentThread for all of 
					// these operations
